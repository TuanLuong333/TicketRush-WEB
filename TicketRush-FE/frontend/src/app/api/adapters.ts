...
const DEFAULT_BANNER_URL = 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?auto=format&fit=crop&w=1280&q=80';
...
export function normalizeMediaUrl(value?: string | null): string | undefined {
  ...
  // No explicit API base: use a relative URL so the browser hits the same
  // origin as the page. On dev this goes through the Vite proxy (which is
  // already configured for /uploads and /api). On prod this assumes the
  // backend is served on the same host or behind a reverse proxy.
  if (!apiBase) return normalizedPath;

  return `${apiBase}${normalizedPath}`;
}

interface SeatMapImageSource {
...
export function getSeatMapImageUrl(source?: SeatMapImageSource | null): string | undefined {
  if (!source) return undefined;

  return normalizeMediaUrl(firstText(
    source.seatMapImageUrl,
    source.seat_map_image_url,
    source.seatMapUrl,
    source.seat_map_url,
    source.seatingChart,
    source.seating_chart,
    source.seatingChartUrl,
    source.seating_chart_url,
    source.seatingChartImageUrl,
    source.seating_chart_image_url,
  ));
}
...
export function adaptEvent(event: ApiEvent): Event {
  ...
  const bannerUrl = normalizeMediaUrl(firstText(
    event.bannerUrl,
    event.banner_url,
    event.imageUrl,
    event.image_url,
  ...
  return {
    ...
    seat_map_image_url: getSeatMapImageUrl(event),
