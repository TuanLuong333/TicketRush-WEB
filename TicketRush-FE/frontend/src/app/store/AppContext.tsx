...
interface AppContextValue {
  ...
  refreshEvents: (includeAll?: boolean) => Promise<void>;
...
// A banner/seat-map URL coming from the server is considered "real" if it's
// either a public URL (http/https) or a path served by the backend (/uploads).
// Only when the event has no real URL do we fall back to a locally cached
// data URL that the admin uploaded in this browser. Otherwise the cache would
// shadow the genuine uploaded image and every event would render the same
// stale data URL.
function isRealMediaUrl(value?: string | null): boolean {
  if (!value) return false;
  if (isInlineImage(value)) return false;
...
function clearLocalBanner(eventId: number) {
  if (typeof window === 'undefined') return;
  try {
    const banners = readLocalBanners();
    if (!(String(eventId) in banners)) return;
    delete banners[String(eventId)];
    window.localStorage.setItem(LOCAL_BANNERS_KEY, JSON.stringify(banners));
  } catch {
    // Ignored - clearing the cache is best-effort.
  }
}
...
function clearLocalSeatMap(eventId: number) {
  if (typeof window === 'undefined') return;
  try {
    const seatMaps = readLocalSeatMaps();
    if (!(String(eventId) in seatMaps)) return;
    delete seatMaps[String(eventId)];
    window.localStorage.setItem(LOCAL_SEAT_MAPS_KEY, JSON.stringify(seatMaps));
  } catch {
    // Ignored - clearing the cache is best-effort.
  }
...
function applyLocalBanner(event: Event): Event {
  ...
  const banners = readLocalBanners();
  const seatMaps = readLocalSeatMaps();
  const cachedBanner = banners[eventId];
  const cachedSeatMap = seatMaps[eventId];

  // Only fall back to the locally cached data URL when the server hasn't
  // provided a real image yet. Otherwise a single old data URL would shadow
  // every freshly uploaded banner / seat map.
  const useBanner = cachedBanner && !isRealMediaUrl(event.banner_url);
  const useSeatMap = cachedSeatMap && !isRealMediaUrl(event.seat_map_image_url);

  if (!useBanner && !useSeatMap) return event;
  return {
    ...event,
    ...(useBanner ? { banner_url: cachedBanner } : {}),
    ...(useSeatMap ? { seat_map_image_url: cachedSeatMap } : {}),
  };
}
...
function extractSeatMapImageUrl(payload: {
  ...
  return getSeatMapImageUrl(payload) || getSeatMapImageUrl(payload.event);
}
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshSeatMap = useCallback(async (eventIdInput: string | number) => {
    ...
    let seatMapImageUrl = extractSeatMapImageUrl(map);
    if (!seatMapImageUrl) {
      try {
        const detail = await apiClient.getEvent(eventId);
        seatMapImageUrl = getSeatMapImageUrl(detail.event);
        if (detail.event && isRealMediaUrl(adaptEvent(detail.event).banner_url)) {
          clearLocalBanner(eventId);
        }
      } catch {
        // Some backends expose zones publicly but keep event detail restricted; keep the current map.
      }
    }
    if (seatMapImageUrl && isRealMediaUrl(seatMapImageUrl)) {
      clearLocalSeatMap(eventId);
    }
    if (seatMapImageUrl) {
  ...
  const refreshEvents = useCallback(async (includeAll = false) => {
    setApiLoading(true);
    try {
      await apiClient.health();
      const apiEvents = includeAll
        ? await apiClient.listAllEvents()
        : (await apiClient.listEvents('PUBLISHED')).data;
      const nextEvents = apiEvents.map(adaptEvent);

      // Once the server is reachable and we know the canonical banner / seat
      // map URL for each event, drop any stale data URLs the admin had cached
      // optimistically. This prevents an old data URL from shadowing the real
      // image on later renders.
      for (const apiEvent of nextEvents) {
        if (isRealMediaUrl(apiEvent.banner_url)) clearLocalBanner(apiEvent.id);
        if (isRealMediaUrl(apiEvent.seat_map_image_url)) clearLocalSeatMap(apiEvent.id);
      }

      const decoratedEvents = nextEvents.map(applyLocalBanner);
      ...
      for (let index = 0; index < maps.length; index += 1) {
        ...
        if (result.status === 'fulfilled') {
          ...
          if (seatMapImageUrl && isRealMediaUrl(seatMapImageUrl)) {
            clearLocalSeatMap(nextEvents[index].id);
          }
          if (seatMapImageUrl) {
            setEvents(prev => prev.map(event => (
  ...
  const refreshOrders = useCallback(async () => {
    if (!apiReady || !apiClient.hasToken()) return;
  ...
  }, [apiReady, refreshAdminOrders, refreshEvents, refreshOrders]);
  ...
  const logout = useCallback(() => {
    ...
    setSelectedSeatIds({});
    if (apiReady) void refreshEvents(false);
  ...
  const addEvent = useCallback(async (event: Event, zones: SeatZone[], layouts: ZoneLayout[]) => {
    ...
    if (apiReady) {
      ...
      // If the server already returned a real URL (uploaded file or external),
      // drop the inline data URL we cached for the optimistic preview so the
      // app shows the canonical image. Only keep the cache when the server
      // didn't accept the file for any reason.
      if (isRealMediaUrl(adaptedFromApi.banner_url)) {
        clearLocalBanner(created.event.id);
      } else if (localBanner) {
        saveLocalBanner(created.event.id, localBanner);
      }
      if (isRealMediaUrl(adaptedFromApi.seat_map_image_url)) {
        clearLocalSeatMap(created.event.id);
      } else if (localSeatMap) {
        saveLocalSeatMap(created.event.id, localSeatMap);
      }
  ...
  const updateEventData = useCallback(async (
    ...
    if (apiReady) {
      ...
      // Drop optimistic data URLs once the server confirms the real URL.
      if (isRealMediaUrl(apiEventData.banner_url)) {
        clearLocalBanner(eventId);
      } else if (localBanner) {
        saveLocalBanner(eventId, localBanner);
      }
      if (isRealMediaUrl(apiEventData.seat_map_image_url)) {
        clearLocalSeatMap(eventId);
      } else if (localSeatMap) {
        saveLocalSeatMap(eventId, localSeatMap);
      }
    } else {
    ...
    setEvents(prev => prev.map(event =>
      event.id === eventId
        ? applyLocalBanner({ ...event, ...data, ...apiEventData, updated_at: updatedAt })
        : event
    ));
  ...
  const cancelHeldSeats = useCallback(async (eventId: string | number, orderIdInput?: number | null): Promise<boolean> => {
    ...
    const now = new Date().toISOString();
    const refreshAfterCancel = async (targetEventId: number) => {
      await Promise.allSettled([
        refreshSeatMap(targetEventId),
        refreshOrders(),
      ]);
    };
    ...
    if (apiReady) {
      try {
        ...
        setHoldExpiry(null);
        setActiveOrderId(null);
        await refreshAfterCancel(cancelled.event_id);
        return true;
      } catch (error) {
        const apiError = error instanceof ApiError ? error : null;
        if (apiError?.status === 409) {
          ...
          setHoldExpiry(null);
          setActiveOrderId(null);
          await refreshAfterCancel(numericEventId);
          return true;
        }
        throw error;
      }
