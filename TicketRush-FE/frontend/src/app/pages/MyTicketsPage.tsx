...
import { AutocompleteInput, uniqueAutocompleteSuggestions } from '../components/AutocompleteInput';
...
export default function MyTicketsPage() {
  ...
  const ownOrders = useMemo(() => {
    if (!user) return [];
    return user.role === 'admin' ? orders : orders.filter(order => order.user_id === user.id);
  }, [orders, user]);

  const searchSuggestions = useMemo(
    () => uniqueAutocompleteSuggestions(ownOrders.flatMap(order => [order.order_code, getEvent(order.event_id)?.title])),
    [getEvent, ownOrders],
  );
        ...
        <div className="mb-7 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="relative w-full md:w-72">
            <AutocompleteInput
              ...
              leadingIcon={<Search size={16} style={{ color: '#94A3B8' }} />}
