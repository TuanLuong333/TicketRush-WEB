...
async function releaseExpiredLocks(existingConn = null) {
  ...
  try {
    ...
    // Read seats that should be released first WITHOUT FOR UPDATE so we don't
    // contend with the row locks that lockSeats / cancelHold / confirmPayment
    // hold on specific seats. We re-check inside the UPDATE to keep things
    // safe even if another transaction modified the rows in the meantime.
...
async function confirmPayment(userId, orderId) {
  ...
  // Read fresh state outside the transaction so we don't hold the connection
  // while running the auxiliary queries that getOrder performs.
...
async function cancelHold(userId, orderId) {
  ...
  try {
    ...
    } else if (new Date(order.expires_at).getTime() < Date.now()) {
